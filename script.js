document.addEventListener("DOMContentLoaded", function (e) {
  function makeRequest(cb) {
    ...
    // constructor 初始化一個 XMLHttpRequest 物件
  ...
  //     const {info} = data;
  // });
  // 執行發 request，把 cb(收到資料後的處理 func) 傳入
  ...
  function getData__Taipei(data) {
    ...
    // 今天時間
    const dateToday = data.cwbopendata.dataset.datasetInfo.validTime.startTime.substr(
      5,
      5
    );
    // 城市名稱(台北) str
    ...
    // 天氣種類
    const typeOfWeather =
      location__Taipei.weatherElement[12].time[0].elementValue[0].value;
    // 溫度相關
    const temperatureAverage =
      location__Taipei.weatherElement[0].time[0].elementValue.value;
    const temperatureMin =
      location__Taipei.weatherElement[4].time[0].elementValue.value;
    const temperatureMax =
      location__Taipei.weatherElement[3].time[0].elementValue.value;
    // 降雨機率
    const RainProb =
      location__Taipei.weatherElement[9].time[0].elementValue.value;
    ...
    // top
    document.querySelector(".top__dateToday").innerHTML =
      dateToday.substr(0, 2) + "/" + dateToday.substr(3, 2);
    ...
    document.querySelector(".top__temperatureAverage").innerHTML =
      temperatureAverage + "°C";
    document.querySelector(".top__temperatureMinToMax").innerHTML =
    ...
    function futureDataUpdate() {
      for (i = 1; i <= 6; i++) {
        let className = ".day" + i;
        document.querySelector(className + " > .bottom__weekDay").innerHTML =
          Future__DateStr(i).substr(0, 2) +
          "/" +
          Future__DateStr(i).substr(3, 2);
        document.querySelector(className + " > .bottom__date").innerHTML =
          weekDay(i).substr(0, 3) + ".";
        document.querySelector(
          className + " > .bottom__temperatureAverage"
        ).innerHTML = Future__temperatureAverage(i) + "°C";
        document.querySelector(
          className + " > .bottom__RainfallProb"
